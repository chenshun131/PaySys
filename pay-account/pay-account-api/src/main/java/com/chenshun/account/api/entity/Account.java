...
import com.chenshun.pay.common.utils.number.AmountUtil;

import java.util.Date;

/**
 * User: mew <p />
 * Time: 18/4/12 16:36  <p />
 * Version: V1.0  <p />
 * Description:  <p />
 */
public class Account extends BaseEntity {
    ...
    public void credit(Double amount) {
        if (this.status == AccountStatusEnum.INACTIVE.getValue()
                || this.status == AccountStatusEnum.CANCELLED.getValue()
                || this.status == AccountStatusEnum.INACTIVE_FREEZE_CREDIT.getValue()) {
    ...
    public void debit(Double amount) {
        if (this.status == AccountStatusEnum.INACTIVE.getValue()
                || this.status == AccountStatusEnum.CANCELLED.getValue()
                || this.status == AccountStatusEnum.INACTIVE_FREEZE_DEBIT.getValue()) {
            throw AccountBizException.ACCOUNT_STATUS_IS_INACTIVE.newInstance("账户状态异常,用户编号{%s},账户状态{%s}", userNo, this.status.intValue()).print();
        }
